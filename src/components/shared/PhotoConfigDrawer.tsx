...
const PhotoConfigDrawer: React.FC<Props> = ({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (file.size > 2 * 1024 * 1024) {
        ...
        // Image is under 2MB; apply light compression for performance
  ...
  const handleUrlChange = async (e: string) => {
    ...
    try {
      ...
      const checkImageSize = () => {
        return new Promise<void>((resolve, reject) => {
          fetch(proxyUrl, { method: "HEAD" })
            ...
            .catch(() => {
              // If size can't be determined, proceed to load the image
              resolve();
            });
        });
      };

      // Check size first before loading
      ...
      toast.error(
        t("upload.invalidUrl", {
          defaultMessage: "Image URL is invalid or inaccessible. Please try a different URL.",
        })
      );
      handleRemovePhoto();
    }
